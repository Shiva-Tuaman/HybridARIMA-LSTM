...
try:
    ...
    # If libraries are missing, show friendly message and stop
...
def init_session():
    """
    Initialize default session state values in a safe way.
    Call this only after Streamlit has started rendering the page (e.g., after st.title).
    """
    defaults = {
        "model_trained": False,
        "arima_model": None,
        "lstm_model": None,
        "scaler": None,
        "best_metrics": {'rmse_7': float('inf'), 'rmse_30': float('inf')},
        "data": None
    }
...
# Title and description (first UI elements — ensures session context is created in deployment)
st.title("📊 LME Copper Price Forecasting System")
st.markdown("**Hybrid ARIMA-LSTM Model for Short-term Copper Price Prediction**")

# Now initialize session state safely
init_session()
...
def train_lstm_model(returns, lookback, epochs):
    ...
    if len(X) == 0:
        raise ValueError("Not enough data to create LSTM sequences. Reduce lookback or increase data length.")
...
def forecast_lstm(model, scaler, recent_data, lookback, steps):
    """Forecast using LSTM model with recursive prediction"""
    predictions = []
...
def rolling_origin_validation(df, arima_model, lstm_model, scaler, lookback, horizon, arima_weight):
    ...
    # Use last 60-90 days for validation depending on data length
...
with col1:
    ...
    if data_source == "Sample Data":
        ...
        st.info("⬆️ Please upload a CSV file or select Sample Data")
        st.stop()
